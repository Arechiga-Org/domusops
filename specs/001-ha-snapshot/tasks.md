...
- [x] T050 [US5] Run quickstart.md scenario 2 (Docker `node:22-slim`, both packed tarballs, `npx --package=… domusops-mcp`) and record the result (pass/fail and the `tools/list` output) in the pull request description (SC-007).
...
- [x] T051 [P] Create `packages/schema/src/json-schema.ts`, exporting `snapshotJsonSchema` (a JSON Schema for the envelope and all three document shapes), and export it from `packages/schema/src/index.ts`. Add `ajv` as a `devDependency` of `@domusops/mcp` (not `@domusops/schema`, which must stay dependency-free and cannot import the mcp encoders), and create `packages/mcp/test/json-schema.test.ts`, which validates `summary`, `standard`, and `full` outputs of `REFERENCE_500` against it (FR-021).
- [x] T052 [P] Create `packages/mcp/README.md` covering:
...
- [x] T053 [P] Remove `--passWithNoTests` from the `test` script in the root `package.json` (research R9), so that deleting all tests fails CI again.
- [x] T054 [P] Update stale references:
...
- [x] T055 [P] Add changesets with `pnpm exec changeset`:
...
- [x] T056 Run `pnpm lint && pnpm typecheck && pnpm test` from the repository root and fix any failure. The `verify` CI job runs the same steps (quickstart.md scenario 1).
- [x] T057 Run quickstart.md scenario 3 against the maintainer's live instance with `detail=summary`, `standard`, and `full`, and scenario 4 (failure smoke test). Record only `ha_version`, the entity count, the `standard` compression ratio, and the wall-clock time in the pull request description, never snapshot content (SC-008). A ratio below 5 blocks release: recalibrate the fixture (T018) and revisit research R6.
